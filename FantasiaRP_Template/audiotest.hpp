...
// sine
float sineFreq = 440.0f;
float reverbSeconds = 3.5f;
...
void setupAudio()
{
    ...
    reverb1.reverbTime(reverbSeconds); // 3.5 second reverb tail
...
void updateAudio()
{
    mixer1.gain(0, (float)cv1 / 4095.0f * 2.0f); // Input gain
    mixer1.gain(1, (float)cv2 / 4095.0f);        // Sine wave gain
    mixer2.gain(0, (float)cv3 / 4095.0f * 2.0f); // Dry signal
    mixer2.gain(1, (float)cv4 / 4095.0f * 2.0f); // Wet signal
